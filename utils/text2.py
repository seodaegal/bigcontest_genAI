...
def recommend_restaurant_from_subset(user_input, top_300_restaurants):
    ...
    all_descriptions = "\n\n".join(
        [f"{restaurant['restaurant_name']}: {restaurant['text2']}" for idx, restaurant in top_300_restaurants.iterrows()]
    )
    ...
    messages = [
        ...
        {
            "role": "user",
            "parts": [f"{top_300_restaurants}는 식당 이름과 해당 식당에 대한 정보가 들어있는 데이터프래임이야. 사용자가 '{user_input}'라고 말했을 때 이 데이터프래임을 참고해서, 여기 있는 식당들 중에서 어떤 식당을 추천할지 3개를 골라주고, 그 이유를 설명해줘. 추천 식당은 최대한 겹치지 않게 해줘."]
        }
    ]
